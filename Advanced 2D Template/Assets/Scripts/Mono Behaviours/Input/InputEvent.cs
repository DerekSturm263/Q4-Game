...
namespace MonoBehaviours.Input
{
    ...
    {
        [SerializeField] protected Types.Input.InputButtonAsset _button;
        public Types.Input.InputButtonAsset Button => _button;
        public void SetButton(Types.Input.InputButtonAsset button) => _button = button;
        ...
        private void Awake()
        {
            _action = _button.Value.Action.action.Clone();
        ...
        public string Type => _button && _button.Value.Action is not null ? _button.Value.Action.action.expectedControlType : string.Empty;
    }
}
