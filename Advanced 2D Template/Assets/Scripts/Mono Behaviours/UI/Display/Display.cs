...
namespace MonoBehaviours.UI
{
    ...
    public abstract class DisplayTextAndImage<T> : Display<T, Types.Miscellaneous.Tuple<UnityEvent<string>, UnityEvent<Sprite>>>
    {
        [SerializeField][TextArea] protected string _format = "{0}";
        [SerializeField] private bool _resizeIfEmpty = true;

        protected abstract Types.Miscellaneous.Tuple<string, Sprite> GetInfo(T item);
        public override void UpdateDisplay(T item)
        {
            var values = GetInfo(item);
